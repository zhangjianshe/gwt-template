...
import cn.mapway.gwt_template.shared.rpc.repository.UpdateGroupMemberRequest;
import cn.mapway.gwt_template.shared.rpc.repository.UpdateGroupMemberResponse;
...
public class UpdateGroupMemberExecutor extends AbstractBizExecutor<UpdateGroupMemberResponse, UpdateGroupMemberRequest> {
    ...
    protected BizResult<UpdateGroupMemberResponse> process(BizContext context, BizRequest<UpdateGroupMemberRequest> bizParam) {
        ...
        DevGroupEntity devGroup= repositoryService.findGroupByName(request.getGroupName());
        assertNotNull(devGroup,"没有发现分组"+request.getGroupName());

        //检查权限
        String unauthorizedMessage="没有授权操作分组"+request.getGroupName();
        DevGroupMemberEntity groupMember= repositoryService.findGroupMemberByMemberId(request.getGroupName(), user.getUser().getUserId());
        ...
        //检查成员
        DevGroupMemberEntity member= repositoryService.findGroupMemberByMemberId(request.getGroupName(), request.getUserId());
        if(member==null){
        ...
        repositoryService.updateGroupMember(request.getGroupName());

        member= repositoryService.findGroupMemberByMemberId(request.getGroupName(), request.getUserId());
