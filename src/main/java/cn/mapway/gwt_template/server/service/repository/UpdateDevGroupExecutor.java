...
import cn.mapway.gwt_template.shared.rpc.repository.UpdateDevGroupRequest;
import cn.mapway.gwt_template.shared.rpc.repository.UpdateDevGroupResponse;
...
public class UpdateDevGroupExecutor extends AbstractBizExecutor<UpdateDevGroupResponse, UpdateDevGroupRequest> {
    ...
    protected BizResult<UpdateDevGroupResponse> process(BizContext context, BizRequest<UpdateDevGroupRequest> bizParam) {
        ...
        DevGroupEntity groupInDB = repositoryService.findGroupByName(devGroup.getName());
        if (groupInDB == null) {
            ...
            //更新
            DevGroupMemberEntity member = repositoryService.findGroupMemberByMemberId(devGroup.getName(), user.getUser().getUserId());
