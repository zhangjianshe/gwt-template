...
public class UpdateRepositoryMemberExecutor extends AbstractBizExecutor<UpdateRepositoryMemberResponse, UpdateRepositoryMemberRequest> {
    ...
    protected BizResult<UpdateRepositoryMemberResponse> process(BizContext context, BizRequest<UpdateRepositoryMemberRequest> bizParam) {
        ...
        CommonPermission permission = repositoryService.findUserPermissionInProject(user.getUser().getUserId(), request.getRepositoryId());
        assertTrue(permission.isAdmin(), "没有操作权限");

        //检查成员
        DevProjectMemberEntity member = repositoryService.findProjectMemberByMemberId(request.getRepositoryId(), request.getUserId());
        if (member == null) {
            member = new DevProjectMemberEntity();
            member.setProjectId(request.getRepositoryId());
        ...
        VwProjectMemberEntity memberView = repositoryService.findProjectMemberViewByMemberId(request.getRepositoryId(), request.getUserId());
