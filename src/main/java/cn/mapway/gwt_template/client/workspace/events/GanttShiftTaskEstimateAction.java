...
public class GanttShiftTaskEstimateAction implements IMouseHandler<GanttHitResult> {
    ...
    public GanttShiftTaskEstimateAction(GanttChart ganttChart) {
        chart = ganttChart;
        result.reset();
    }
    ...
    public void onMouseMove(MouseMoveEvent event) {
        if (mouseDown) {
            ...
            if (result.getGanttItem() != null) {
                result.getGanttItem().offsetEstimateTime(chart.getDocument(), deltaX);
            }
            chart.redraw();
        }
    }
