...
import cn.mapway.gwt_template.client.desktop.DesktopEditor;
import cn.mapway.gwt_template.client.desktop.DesktopItem;
...
import static cn.mapway.gwt_template.client.dashboard.ShortcutWidget.MODULE_CODE;
...
public class ShortcutWidget extends BaseAbstractModule {
    public static final String MODULE_CODE = "widget_shortcut";
    private static final ShortcutWidgetUiBinder ourUiBinder = GWT.create(ShortcutWidgetUiBinder.class);
    @UiField
    HTMLPanel root;
    ...
    private void renderItem(QueryDesktopResponse data) {
        root.clear();
        for (DesktopItemEntity desktopItem : data.getItems()) {
            DesktopItem item = new DesktopItem();
            item.setData(desktopItem);
            item.addCommonHandler(itemHandler);
            root.add(item);
        }
    }

    interface ShortcutWidgetUiBinder extends UiBinder<HTMLPanel, ShortcutWidget> {
    }
